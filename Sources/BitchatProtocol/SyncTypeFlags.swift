...
public struct SyncTypeFlags: OptionSet, Sendable {
    ...
    public static let announce = SyncTypeFlags(messageTypes: [.announce])
    public static let message = SyncTypeFlags(messageTypes: [.message])
    public static let fragment = SyncTypeFlags(messageTypes: [.fragment])
    public static let fileTransfer = SyncTypeFlags(messageTypes: [.fileTransfer])

    public static let publicMessages = SyncTypeFlags(messageTypes: [.announce, .message])

    public init(messageTypes: [MessageType]) {
        var raw: UInt64 = 0
    ...
    public func union(_ other: SyncTypeFlags) -> SyncTypeFlags {
        SyncTypeFlags(rawValue: rawValue | other.rawValue)
    }

    public func intersection(_ other: SyncTypeFlags) -> SyncTypeFlags {
        SyncTypeFlags(rawValue: rawValue & other.rawValue)
    }

    public func toMessageTypes() -> [MessageType] {
    ...
    public static func decode(_ data: Data) -> SyncTypeFlags? {
